           ...
           <div class="container">
               <div class="row">
                   @foreach($category->sub_categories as $sub_c)
                       <div class="col-md-3">
                           <div class="single-category-title"><a href="{{ route('search', [ $current_country['country_code'],  'category' => 'cat-'.$sub_c->id.'-'.$sub_c->category_slug]) }}"><i class="fa fa-folder-open-o"></i> {{$sub_c->category_name}}</a></div>
                       </div>
                   @endforeach
               </div>
                        ...
                        <div class="col-md-3">
                            <div class="home-cat-box">
                                <div class="home-cat-box-title">
                                    <h3><a href="{{ route('search', [ $current_country['country_code'],  'category' => 'cat-'.$top_cat->id.'-'.$top_cat->category_slug]) }}"> {{$top_cat->category_name}} </a> </h3>
                                </div>
                                @if($top_cat->sub_categories->count())
                                    <div class="home-cat-box-content">
                                        <ul>
                                            @foreach($top_cat->sub_categories as $sub_cat)
                                                <li><a href="{{ route('search', [ $current_country['country_code'],  'category' => 'cat-'.$sub_cat->id.'-'.$sub_cat->category_slug]) }}">&raquo; {{$sub_cat->category_name}}</a> </li>
                                            @endforeach
                                        </ul>
